...
const Seedance = () => {
  ...
  useEffect(() => {
    let ratePerSecond = 0.012;
    if (resolution === '720p') ratePerSecond = 0.024;
    else if (resolution === '1080p') ratePerSecond = 0.048;
    const estimatedCost = duration * ratePerSecond;
    setRunCost(parseFloat(estimatedCost.toFixed(3)));
  }, [duration, resolution]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const submitUrl = 'https://api.wavespeed.ai/api/v3/bytedance/seedance-v1-pro-fast/image-to-video';
      const payload = {
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-semibold text-gray-900">Jon Mac AI - Seedance</h1>
          </div>
        ...
        {activeTab === 'API' ? (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <p className="text-xs text-gray-500 mt-2">
              Enter your WaveSpeed API key here. Uses seedance-v1-pro-fast/image-to-video. Key is required.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
              ...
              <div>
                <label className="flex items-center gap-2 text-sm font-medium text-gray-700 mb-2">
                  aspect_ratio <Info className="w-3.5 h-3.5 text-gray-400" />
                </label>
                <select
                  value={aspectRatio}
              ...
              <div>
                <label className="flex items-center gap-2 text-sm font-medium text-gray-700 mb-2">
                  camera_fixed <Info className="w-3.5 h-3.5 text-gray-400" />
                </label>
                <div className="flex items-center gap-3">
                  <div
                    onClick={() => setCameraFixed(!cameraFixed)}
                    className={`w-5 h-5 rounded border flex items-center justify-center cursor-pointer transition-colors ${cameraFixed ? 'bg-blue-600 border-blue-600' : 'bg-white border-gray-300'}`}
                  >
                    {cameraFixed && <Check className="w-3 h-3 text-white" />}
                  </div>
                  <span className="text-xs text-gray-500">Whether to fix the camera position.</span>
                </div>
              </div>
