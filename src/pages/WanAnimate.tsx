...
const WanAnimate = () => {
  ...
  const [inputVideo, setInputVideo] = useState<InputVideo | null>(null);
  ...
  const handleVideoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (file.size > 15 * 1024 * 1024) {
        setError('Video file is too large. Please keep it under 15MB for browser-based processing.');
        return;
      }
      const reader = new FileReader();
      reader.onloadend = () => {
      ...
      try {
        ...
        if (status === 'completed' || status === 'succeeded') {
          ...
          if (outputs && outputs.length > 0) {
            ...
            if (i === 299) throw new Error('Completed but no output found.');
            continue;
          }
  ...
  const handleGenerate = async () => {
    if (!apiKey) {
      setError('Please provide a WaveSpeed API Key in the API tab.');
      setActiveTab('API');
      return;
    }
    if (!inputImage) return setError('Please upload a reference image.');
    if (!inputVideo) return setError('Please upload a source video.');
    ...
    try {
      const submitUrl = 'https://api.wavespeed.ai/api/v3/wavespeed-ai/wan-2.2/animate';
      const payload = {
        image: `data:${inputImage.mimeType};base64,${inputImage.base64}`,
        video: `data:${inputVideo.mimeType};base64,${inputVideo.base64}`,
        prompt: prompt,
        mode: mode,
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-semibold text-gray-900">Jon Mac AI - Wan Animate</h1>
          </div>
        ...
        {activeTab === 'API' ? (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <p className="text-xs text-gray-500 mt-2">
              Enter your WaveSpeed API key here. Uses wavespeed-ai/wan-2.2/animate. Key is required.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
              <div>
                <label className="flex items-center gap-2 text-sm font-medium text-gray-700 mb-2">
                  prompt <Info className="w-3.5 h-3.5 text-gray-400" />
                </label>
                <textarea
                  value={prompt}
                  onChange={(e) => setPrompt(e.target.value)}
                  placeholder="Describe the animation..."
                  className="w-full p-3 border border-gray-300 rounded-lg text-sm min-h-[80px] focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none resize-none"
                />
              </div>
              ...
              <div>
                ...
                {inputVideo ? (
                  <div className="space-y-2">
                    <div className="flex items-center justify-between p-2 bg-gray-50 rounded-lg">
                      <span className="text-sm text-gray-600 truncate flex-1">{inputVideo.name}</span>
                      <Trash2
                        className="w-4 h-4 text-gray-400 hover:text-red-500 cursor-pointer ml-2"
                    ...
                    <div className="flex items-center gap-2 p-2 bg-gray-50 rounded-lg">
                      <Film className="text-gray-400 w-8 h-8" />
              ...
              <div>
                <label className="flex items-center gap-2 text-sm font-medium text-gray-700 mb-2">
                  mode <Info className="w-3.5 h-3.5 text-gray-400" />
                </label>
                <select
                  value={mode}
              ...
              <div>
                <label className="flex items-center gap-2 text-sm font-medium text-gray-700 mb-2">
                  resolution <Info className="w-3.5 h-3.5 text-gray-400" />
                </label>
                <select
                  value={resolution}
              ...
              <div>
                <label className="flex items-center gap-2 text-sm font-medium text-gray-700 mb-2">
                  seed <Info className="w-3.5 h-3.5 text-gray-400" />
                </label>
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 p-2 border border-gray-300 rounded-md text-sm bg-white focus:border-blue-500 outline-none"
                  />
                  <button onClick={randomizeSeed} className="p-2 border border-gray-300 rounded-md hover:bg-gray-50">
            ...
            <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <div className="mt-4 flex items-center gap-2 text-xs text-gray-500">
                ...
                <span>Your request will cost ${runCost} per run. Video + Image animate generation.</span>
              </div>
            </div>
          </div>
        )}
