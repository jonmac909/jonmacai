...
export function render(d) {
  const edit = d.editing.rows.map((r) => `<div class="r"><div><strong>${r.title}</strong><small>${r.sub}</small>${pg(r.pct)}</div><div class="right">${pill(r.pill, r.pillCls || '')}${r.btn ? btn(r.btn, { msg: r.msg, cls: 'line', sm: true, kind: r.kind, payload: r.payload }) : ''}</div></div>`).join('');
  const ready = d.ready.rows.map((r) => `<div class="r"><div><strong>${r.title}</strong><small>${r.sub}</small></div><div class="right">${btn('Send notes', { msg: 'Notes box opened', cls: 'line' })}${btn('Watch', { msg: 'Opened the cut for review', href: r.href })}</div></div>`).join('');
  ...
  <div class="split">
    ...
    <div class="stackcol">
      <div class="card pad">
        ...
        <div class="drop" data-upload="1"><svg class="ic" style="width:1.8rem;height:1.8rem;color:var(--accent)"><use href="#i-up"/></svg><strong>${d.upload.drop}</strong><span>${d.upload.sub}</span>${btn(d.upload.btn, { })}<input id="cc-upload" type="file" accept="video/*" hidden></div>
      </div>
      <div class="card pad">
