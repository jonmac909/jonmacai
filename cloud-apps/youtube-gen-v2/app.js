...
(function () {
  ...
  const PICK_KEY = "jonmac_youtube_gen_v2_template_picks_v1";
  ...
  function chooseTemplate(sourceId, queryTemplate = "") {
    const row = allRows.find((item) => item.id === sourceId);
    const templateId = pickTemplate({
      queryTemplate,
      sourceId,
      stored: loadPicks(),
      recommended: row ? recommendTemplate(row).id : "",
    });
    if (templateId) savePicks(rememberTemplate(loadPicks(), sourceId, templateId));
    return templateId;
  }
  ...
  app.addEventListener("click", (event) => {
    ...
    else if (target.dataset.remake) { state.drawerSourceId = target.dataset.remake; state.drawerTemplateId = chooseTemplate(target.dataset.remake); render(); }
    else if (target.hasAttribute("data-close-drawer")) { state.drawerSourceId = null; render(); }
    else if (target.dataset.drawerTemplate) { state.drawerTemplateId = target.dataset.drawerTemplate; if (state.drawerSourceId) savePicks(rememberTemplate(loadPicks(), state.drawerSourceId, state.drawerTemplateId)); render(); }
  ...
  if (bootQ.get("remake")) {
    ...
    state.drawerTemplateId = chooseTemplate(state.drawerSourceId, bootQ.get("template") || "");
  }
  render();
  Promise.resolve(hydrateProjects()).then(() => {
    if (!state.drawerSourceId) return;
    const existing = state.projects.find((p) => p.source && p.source.id === state.drawerSourceId);
    if (!existing || !state.drawerTemplateId) return;
    existing.templateId = state.drawerTemplateId;
