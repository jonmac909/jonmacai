...
test('remake button keeps a stored template instead of the recommendation', () => {
  const stored = rememberTemplate({}, 'vid', 'winner_teardown');
  assert.equal(pickTemplate({ sourceId: 'vid', stored, recommended: 'brand_build' }), 'winner_teardown');
  assert.equal(pickTemplate({ queryTemplate: 'brand_build', sourceId: 'vid', stored, recommended: 'trend_to_revenue' }), 'brand_build');
  assert.equal(rememberTemplate(stored, 'vid', 'brand_build').vid, 'brand_build');
});
